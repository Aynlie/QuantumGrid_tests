...
def render_dashboard():
    ...
    # --- Rescale raw demand/solar SHAPE onto this feeder's own real
    # nominal load and an assumed PV capacity -- same approach main.py
    # uses. bundle["demand_pu"]/["solar_pu"] are raw Kaggle-scale values
    # (a whole utility system + a utility-scale solar farm), far larger
    # than this 10 MVA feeder; feeding them in directly makes every
    # renewable-fraction calculation meaningless (division of unrelated
    # scales). Only the SHAPE (normalized pattern) is used here.
    PV_CAPACITY_PU = 0.35  # keep in sync with main.py's PV_CAPACITY_PU
    nominal_total_load_pu = sum(b.P_load_pu for b in bundle["graph"].buses.values())
    ...
    with col1:
        ...
        st.line_chart(demand_pu)
    with col2:
        st.subheader("Solar Generation (p.u.)")
        st.line_chart(solar_pu)
    ...
    with col3:
        ...
        st.metric("Renewable contribution (peak demand hour)",
                   f"{compute_renewable_fraction(total_renewable, total_load):.1f}%")
        st.caption(f"Snapshot at {peak_hour} — solar output is near zero "
                    f"this late; peak demand and peak solar don't align.")
        st.metric("Renewable contribution (24h average)",
                   f"{avg_renewable_frac:.1f}%")
    ...
    if st.button("Simulate Fault"):
        net_injection = {b: demand_pu.iloc[-1] for b in bundle["graph"].buses}
...
if __name__ == "__main__":
    ...
    if _running_in_streamlit():
        # Launched via `streamlit run dashboard.py` -- actually render the UI.
        render_dashboard()
    else:
        # Launched via `python dashboard.py` -- test the metrics layer
        # directly, no Streamlit/browser needed.
